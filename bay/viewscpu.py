...
class ToolViewSet(viewsets.ModelViewSet):
	...
	#    return Tool.objects.filter(bay_number="1")
	bay_num = (self.request.query_params.get('bid'))
	    ...
	    return Tool.objects.filter(bay_number=bay_num)
	else:
...
#	tool_obj = Tool.objects.get(bay_number=pk)
	...
	try:
	    ...
	    raise AttributeError(list(bay_number))
	...
	for project in projects:
	    for user in users:
         	ToolUtil = Logging.objects.filter (created_on__gte=start_date, created_on__lte=today,tool_id=pk, user=user,project=project).order_by('created_on')
                # today = timezone.now()
...
def utilization_tool(tool_id, start_date=None,end_date=None):
    ...
    if con > start_date:
	start_date = con
    ToolUtil = Logging.objects.filter(created_on__gte=start_date, created_on__lte=today, tool_id=tool_id).order_by('created_on')
    total_seconds = ((today - start_date).total_seconds())
    InUse_Time = 0
    Maintenance_Time = 0
    Installation_Time = 0
   ...
   # format.set_bold()
	...
	total_mn += time['Maintenance_Time']
        ...
        row = [
	    	...
	    	end.strftime('%Y-%m-%d %H:%M') if end else end_date('%Y-%m-%d %H:%M'),
...
def project_utilization(project, start_date=None,end_date=None):
    ...
    if len(ToolUtil) ==0:
	   ...
	   status = obj.last().status
           if(status == "PR"):
               ...
               Maintenance_Time = total_seconds
    '''
    previous_log = Logging.objects.filter(created_on__lt=start_date,project=project).order_by('created_on').last()
    second_date = today
    next_log = Logging.objects.filter(created_on__gt=second_date,project=project).order_by('created_on').first()
    data = {}
    data['date'] = start_date
    data[u'PR'] = 0
    data[u'IN'] = 0
    data[u'MA'] = 0
    data[u'ID'] = 0
    logging = Logging.objects.filter(created_on__gte=start_date,created_on__lt=second_date,project=project).order_by('created_on')
    for i in range(0,len(logging)):
        log = logging[i]
        if i>0:
            previous_log = logging[i-1]
