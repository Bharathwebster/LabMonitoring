	...
	start_date = datetime.date.today()
	    ...
	    for project in projects:
		 ...
		 if len(ToolUtil) ==0:
	   	 ...
	   	 utilzation_data['Project name'] = project.name
...
def utilization(tool_id, start_date=None,end_date=None):
    ...
    total_percent = InUse_Time + Idle_Time + Maintenance_Time + Installation_Time+ 0.01
...
def export_project_xls(request):
    try:
        ...
        start_date = time_zone.localize(datetime.datetime.strptime('2017-3-31','%Y-%m-%d'))
    try:
        time_zone = pytz.timezone(settings.TIME_ZONE)
        end_date_str = request.GET.get("end_date",None)
        end_date = time_zone.localize(datetime.datetime.strptime(end_date_str,'%Y-%m-%d'))
...
def export_user_xls(request):
    ...
    response['Content-Disposition'] = 'attachment; filename='+filename
    wb = xlwt.Workbook(encoding='utf-8')
    #ws = wb.add_sheet("Users Report",cell_overwrite_ok=True)
