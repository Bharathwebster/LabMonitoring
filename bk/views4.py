...
class ToolViewSet(viewsets.ModelViewSet):
	...
	qtr = list(rrule(freq=MONTHLY,bymonth=(2,5,8,11), count=4, dtstart=start_date))
        Q1 = qtr[0]
        Q2 = qtr[1]
        Q3 = qtr[2]
        Q4 = qtr[3]
	tools = Tool.objects.all()
        ...
        Maintenance_Time = (total_maintenance/total_seconds_console)*100
        Idle_Time = 100 - (InUse_Time+Installation_Time+Maintenance_Time)
        time = {"Productive_Timie":InUse_Time,"Idle_Time":Idle_Time,"Maintenance_Time":Maintenance_Time,"Installation_Time":Installation_Time,"Start_DATE":start_date,"EndDate":end_date,"Q1":Q1,"Q2":Q2,"Q3":Q3,"Q4":Q4}
        return Response(time)
...
def export_project_xls(request):
    ...
    columns = [
        ...
        (u"Start Date", 8000),
        (u"End Date", 8000),     
        (u"Project utilization in hours", 8000),
    ...
    for pk_val in tool:
	    ...
	    Overall_MA += result[i][u'MA']
            try :
                ...
                out.append(result[i])
        Overall_PR = (Overall_PR/(Overall_PR + Overall_ID + Overall_IN + Overall_MA))*100;
	Overall_IN = (Overall_IN/(Overall_PR + Overall_ID + Overall_IN + Overall_MA))*100;
	Overall_ID = (Overall_ID/(Overall_PR + Overall_ID + Overall_IN + Overall_MA))*100;
	Overall_MA = (Overall_MA/(Overall_PR + Overall_ID + Overall_IN + Overall_MA))*100;
